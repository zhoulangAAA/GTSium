...
namespace Sium
{
    public partial class DPindex : Form
    {
        StringBuilder FPPcokk;
        HttpPost PostServer = new HttpPost();
        SQLServer Ms = new SQLServer();
        string CcateId, CTit, CItemid, CDateTime, TID;
        DateTime getdatTime;
        public DPindex(StringBuilder FPPcok, string cateId, string shopTit, string ItemID,string Tid)
        {
            ...
            TID = Tid;
            this.GetdateIN.MaxDate = DateTime.Now.AddDays(-1);
        }
        ...
        public void DataBinderTom(string IshopItemID)
        {
            ...
            DataTable getKeyword2 = Ms.runSQLDataSet(sqlU, "ss").Tables[0];
            getKeyword2.Columns.Remove("ItemID");
            getKeyword2.Columns.Remove("指数");
            ultraGrid3.DataSource = getKeyword2;
        ...
        private void GetdateIN_ValueChanged(object sender, EventArgs e)
        {
            string BegDate = GetdateIN.Value.ToString("yyyy-MM-dd");
            string shopNameItemd = CTit + CItemid;
            DelUVTom(shopNameItemd);
            getUVList("uv", BegDate, CcateId, CItemid, shopNameItemd);
            getUVList("payByrCntIndex", BegDate, CcateId, CItemid, shopNameItemd);
        ...
        {
            if (GetdateIN.Value.Day==DateTime.Now.AddDays(-1).Day)
            {
                MessageBox.Show("查询时间不能大于：" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd"));
            }
            else
            {
                GetdateIN.Value = GetdateIN.Value.AddDays(+1);
                string BegDate = GetdateIN.Value.ToString("yyyy-MM-dd");
                string shopNameItemd = CTit + CItemid;
                DelUVTom(shopNameItemd);
                getUVList("uv", BegDate, CcateId, CItemid, shopNameItemd);
        ...
        private void DPindex_Load(object sender, EventArgs e)
        {
            NowKeyWordsgetai();
            getUVList();
            PayList();
